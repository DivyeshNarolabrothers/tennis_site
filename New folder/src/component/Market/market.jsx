...
function Market() {
  ...
  useEffect(() => {
    const fetchBannerText = async () => {
      try {
        ...
        const response = await axios.get("http://localhost:9999/users/banner_text", {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });

        // Assuming response.data is an array, get the first item's text
  ...
  useEffect(() => {
    ...
    const interval = setInterval(fetchMarketStatus, 1000000);
  ...
  const fetchPlayersAndMarkSelected = async () => {
    ...
    try {
      ...
      const playersResponse = await axios.get(
        "http://localhost:9999/users/players",
        { headers: { Authorization: `Bearer ${Token}` } }
      );
      ...
      const selectedTeamResponse = await axios.get(
        `http://localhost:9999/users/selected-teamlist/${userId}`,
        { headers: { Authorization: `Bearer ${Token}` } }
      );
  ...
  const handleSellConfirm = async () => {
    ...
    try {
      ...
      const response = await axios.patch(
        `http://localhost:9999/users/sell-player/${userId}`, // Ensure this endpoint is correct
        requestBody,
        {
          headers: {
            Authorization: `Bearer ${Token}`,
      ...
      console.error(error);
      alert("You do not own enough shares to sell that quantity. Please try again");
    } finally {
  ...
  const confirmTeam = async () => {
    try {
      ...
      const response = await axios.post(
        `http://localhost:9999/users/updateTeam/${userId}`,
        { addPlayers },
        {
          headers: {
            Authorization: `Bearer ${Token}`,
  ...
  const fetchSelectedTeamList = async () => {
    try {
      ...
      if (response.data.status) {
        ...
        const baseUrl = "http://localhost:9999/images/"; // Adjust this URL to your actual image base URL
          ...
          <div className="card-header1">
            <div className="d-flex">
              ...
              <div className="right-section1">
                ...
                <p>
                  Available: <span>{credits.toFixed(2)}</span>
                </p>
            ...
            <div className="d-flex">
              ...
              <button className="button" style={{marginRight:'10px'}} onClick={()=>navigate('/home')}>Home</button>
                  ...
                  {Array.isArray(players) && players.length > 0 ? (
                    players.map((player) => (
                      <tr key={player._id}>
                        <td>
                          <span
                            className={`circle ${player.selected ? "green-circle" : ""
                              }`}
                            style={{
                              width: "10px",
                              height: "10px",
...
>
  <Modal.Header>
    <Modal.Title>
      <h5 className="modal-h5">To Buy A Player Click The Buy Button(s).</h5>
      <h4 className="modal-h1">Player Selection</h4>
      <div>
        <p className="modal-p">
          Sort by:
          ...
          players.map((player) => (
            <tr key={player._id}>
              <td>
                <span
                  ...
                  style={{
                    ...
                    height: "10px",
                    borderRadius: "50%",
                    display: player.selected ? "inline-block" : "none",
                    margin: "0px 8px",
                  }}
                ></span>
                {player.name}
              ...
              <td>
                ...
                {player.selected && player.share_quantity > 0 && (
                  <div style={{ display: "flex", marginTop: "5px" }}>
                    ...
                    >
                      (
                      {player.valueDifference
                        ? player.valueDifference * player.share_quantity
                        : 0}
                      )
                    </div>
                  </div>
...
>
  ...
  >
    <table style={{ backgroundColor: "transparent", width: "100%" }}>
      ...
      <tbody>
        {Array.isArray(players) && players.length > 0 ? (
          // Sort players so selected ones are at the top
          [...players]
      ...
      <table>
        ...
        <tbody>
          {selectedPlayers.map((player) => {
            const shares =
              playerShares[player._id] ||
              playerShares[player.player_id] ||
              player.share_quantity ||
              0;

            const totalCost = (shares * player.value).toFixed(2);
            const totalTeamCost = selectedPlayers.reduce(
              (acc, p) =>
                acc +
                ((playerShares[p._id] || playerShares[p.player_id] || 0) *
                  p.value),
              0
            );
            ...
            return (
              <tr key={player._id || player.player_id}>
          ...
          <tr>
            ...
            <td className="confirm-td">
              {selectedPlayers.reduce(
                (acc, player) =>
                  acc + (playerShares[player._id] || player.share_quantity || 0),
                0
              )}
            </td>
            <td className="confirm-td">
              {selectedPlayers
                .reduce(
                  (acc, player) =>
                    acc +
                    (playerShares[player._id] || player.share_quantity || 0) *
                      player.value,
                  0
                )
                .toFixed(2)}
                    ...
                    <div key={index} className="icon1">
                      {icon ? (
                        <div className="player-container">
                          <img
                            src={`http://localhost:9999/images/${icon.profile_image}`}
                            alt="Player Profile"
