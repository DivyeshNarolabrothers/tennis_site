...
const FaqPage = () => {
  ...
  return (
    <div className='container'>
       <button className="button" style={{marginBottom:'10px',marginTop:'10px'}} onClick={()=>navigate('/home')}>Home</button>
      <Accordion defaultActiveKey="0" style={{ padding: '10px 0px' }}>
        {faqData.map((faq, index) => (
          <Accordion.Item eventKey={index.toString()} key={faq._id}>
            <Accordion.Header style={{ color: 'black', whiteSpace: 'pre-wrap' }}>
              {faq.question}
            </Accordion.Header>
            <Accordion.Body style={{ color: 'gray', whiteSpace: 'pre-wrap' }}>
              {faq.answer}
            </Accordion.Body>
          </Accordion.Item>
        ))}
