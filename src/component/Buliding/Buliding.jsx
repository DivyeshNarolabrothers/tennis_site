...
function Building() {
  ...
  const fetchPlayers = async () => {
    const Token = localStorage.getItem("user_token");
    axios
      .get("http://35.200.147.33:9999/users/players", {
        headers: { user_token: Token },
      })
      .then((response) => {
  ...
  const confirmTeam = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `http://35.200.147.33:9999/users/add-team/${userId}`,
        formData,
        { headers: { user_token: Token } }
      );
                  ...
                  {Array.isArray(players) && players.length > 0 ? (
                    players.map((player) => (
                      <tr key={player._id}>
                        <td>
                          <img
                            src={`http://35.200.147.33:9999/images/${player.profile_image}`}
                            alt="Player Profile"
                      ...
                      {icon ? (
                        <>
                          ...
                          <div className="player-container">
                            {" "}
                            <img
                              src={`http://35.200.147.33:9999/images/${icon.image}`}
                              alt="Player Profile"
