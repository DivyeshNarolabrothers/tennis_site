...
//changes
  ...
  useEffect(() => {
    ...
    fetchMarketStatus();
    const interval = setInterval(fetchMarketStatus, 10000000);
  ...
  const handleBuyPlayer = (playerId, iconIndex) => {
    ...
    if (isMarketFrozen) {
      showAutoError("Player purchases are currently disabled");
      return;
    }
  ...
  const confirmTeam = async () => {
    ...
    try {
      ...
      const playersData = selectedPlayers.map((player) => ({
        id: player._id,
        share_quantity: playerShares[player._id] || 0,
        name: player.name,
        value: player.value,
      }));
      ...
      // Update credits with the closing_credits from the response
            ...
            <div className="footer">
              ...
              <div className="d-flex">
                ...
                <div className="first1">
                  {" "}
                  <h5>{selectedIcons.filter(Boolean).length}/8</h5>{" "}
